...
class BaseProjectForm(FlaskForm):
    ...
    related_skills = SelectMultipleField('Related Skills', choices=[], coerce=str)
    ...
    def validate_is_featured(self, field):
        if field.data:
            ...
            if featured_count >= 3:
                raise ValidationError('Only 3 projects can be featured at a time.')
...
class UpdateProjectForm(BaseProjectForm, ImageUploadForm):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.category.choices = [(category.id, category.name) for category in ProjectCategory.query.all()]
        self.featured_order.choices = [(i, str(i)) for i in range(1, 4)]
        self.status.choices = [(status.value, status.name) for status in ProjectStatus]
        self.related_skills.choices = [(skill.id, skill.name) for skill in Skill.query.all()]
